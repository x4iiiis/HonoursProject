...
Artificial neural networks, commonly referred to simply as neural networks, are a rough representation of the human brain. Human brains can be thought of as highly complex and non-linear systems for processing information in parallel\cite{7}. 
...
$w \cdot x+b$ is of modest size that there's much deviation from the perceptron model\cite{4}.
...
Representation refers to specifying which genotypes equate to each phenotype\cite{5}. For example, if an integer is the solution to a problem, it is the phenotype, and a binary representation of that particular integer would be the genotype relating to that phenotype. 
...
The role of an evaluation function (or fitness function) to encourage improvements by defining what an improvement is\cite{5}. This lays the foundation for selection. 

\subsubsection{Selection}
The process of choosing individuals, based on their fitness, from the population to become parents to the next generation of individuals is called selection\cite{5}. This is the driving force behind progressive evolution as it biases selection towards individuals of higher quality. 

\subsubsection{Crossover}
Sometimes referred to as recombination, crossover is an operation that merges genes from two parent genotypes together into one or two offspring genotypes\cite{5}. 
...
When applied to a genotype, mutation returns a slightly mutated offspring\cite{5}. It can create new genes in the population, which in turn diversifies the population. 
...
\enquote{NEAT is a method for evolving speciated neural networks of arbitrary structures and sizes. NEAT leverages the evolution of structure to make neuroevolution more efficient}\cite{6}.
...
\subsection{How intelligent is it?}
What did the evolved strategy look like
-Did it come up with something cheap and easy like the mario thing where it jumps all the time, or does it resemble a strategy a human player might use
